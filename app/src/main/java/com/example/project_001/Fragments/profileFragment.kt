...
import com.example.project_001.AccountSettingsMainActivity2
...
class profileFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (profileId == firebaseUser.uid){
            view.findViewById<TextView>(R.id.edit_account_settings_btn).text = "Edit Profile"
        }
        else if (profileId != firebaseUser.uid){
            checkFollowAndFollowingButtunStatus()
        }

        view.findViewById<Button>(R.id.edit_account_settings_btn).setOnClickListener {
            val getButtonText = view.findViewById<TextView>(R.id.edit_account_settings_btn).text.toString()
            when{
                getButtonText == "Edit Profile" -> startActivity(Intent(context,AccountSettingsMainActivity2::class.java))
    ...
    private fun checkFollowAndFollowingButtunStatus() {
            ...
            followingRef.addValueEventListener(object : ValueEventListener{
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.child(profileId).exists()){
                        view?.findViewById<TextView>(R.id.edit_account_settings_btn)?.text = "Following"
                    }
                    else {
                        view?.findViewById<TextView>(R.id.edit_account_settings_btn)?.text = "Follow"
                    }
                }
        ...
        usersRef.addValueEventListener(object : ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()){
                    ...
                    Picasso.get().load(user!!.getImage()).placeholder(R.drawable.profile).into(view?.findViewById(R.id.pro_image_profile_fragment))
